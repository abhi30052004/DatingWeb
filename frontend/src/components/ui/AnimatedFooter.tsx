...
export function AnimatedFooter({
  ...
  useEffect(() => {
    ...
    const loadHand = (src: string, canvas: HTMLCanvasElement, direction: 1 | -1) => {
      ...
      image.onload = init;
      image.onerror = () => { console.warn(`Failed to load ${src}`) };
      image.src = src;
      ...
      <div className="absolute inset-x-0 bottom-0 flex items-end justify-center gap-4 p-8 z-10">
        {headingLines.map((word, wi) => (
          <h2
            key={`${word}-${wi}`}
            aria-label={word}
            className="overflow-hidden font-bold leading-none tracking-tight pb-[0.15em] -mb-[0.15em] text-white"
            style={{ fontSize: "clamp(3rem, 10cqw, 11rem)" }}
          >
